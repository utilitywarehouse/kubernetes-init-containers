...
# We only want to continue if a replica set configuration does not exist.
...
mongo --quiet --eval "
rs.initiate( { _id : \"${REPL_SET}\", members: [ { _id : 0, host : \"${name}\" } ] })"

# Wait for mongodb to understand that it is the current master.
sleep 10
...
# Reconfigure the cluster node priorities.
NEWLINE=$'\n'
